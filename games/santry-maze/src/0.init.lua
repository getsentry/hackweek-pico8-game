...
function _update()
  ...
  -- update movement timer for sprite animation
  if player_x_moved != 0 or player_y_moved != 0 then
    movement_timer += 1
  end
...
function _draw()
  if state == -1 then
    ...
    if goal_animation_active then
      ...
      -- alternate between sprite 1 and sprite 17 every 8 frames during animation
      local sprite_id = (flr(goal_animation_timer / 8) % 2 == 0) and 1 or 17
      
      -- draw player sprite with color replacement
      pal(player_color, rainbow_color)  -- replace player color with rainbow color
      spr(sprite_id,player.x_pos,player.y_pos)
      pal()  -- reset palette
    else
      -- alternate between sprite 1 and sprite 17 every 8 frames during movement
      local sprite_id = (flr(movement_timer / 8) % 2 == 0) and 1 or 17
      spr(sprite_id,player.x_pos,player.y_pos)
    end
